...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # returns starting position and an empty set representing no corners reached yet.
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if number of corners added equals to total number of corners, goal state is reached.
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if next_position in self.corners:  # if next position comes to a corner
                    # if this corner is not yet reached, added to corners list
                    if not next_position in state[1]:
                        next_state = (
                            next_position, (state[1] + (next_position,)))
                # else added to successors
...
def cornersHeuristic(state, problem):
    ...
    heuristic = 0
    # initializing a list of unvisited corners
    unvisited_corners = []
    for corner in corners:
        if not corner in visited_corners:
            unvisited_corners.append(corner)

    # While unvisited corners is not empty
    while unvisited_corners:
        # create list of manhattan distances from curr position to unvisited corner
        distances = []
        ...
        nearest_corner = unvisited_corners[distances.index(
            min_distance)]   # the nearest corner extracted
...
def foodHeuristic(state, problem):
    ...
    # optimization: using maze distance instead of manhattan distance
    for food in foodList:
        distances.append(mazeDistance(
            position, food, problem.startingGameState))
    # max distance as heuristic
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # in my implementation of A* search the food grid is cast to a list at first, making it incompatible in this scenario.
        # will use bfs for this problem, since it's the only complete search algorithm left to use
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        isFood = self.food  # true if there is food, false if not
        if isFood[x][y] == True:
            return True
        return False
        # util.raiseNotDefined()
