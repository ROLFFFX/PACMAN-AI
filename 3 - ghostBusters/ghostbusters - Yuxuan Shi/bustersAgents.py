    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # determine the most likely positions for each uncaptured ghosts based on belief distribution
        ...
        for ghostPositionDistribution in livingGhostPositionDistributions:
            ...
            mostLikelyPosition = ghostPositionDistribution.argMax()
            mostLikelyGhostPositions.append(mostLikelyPosition)

        # get the distance to each of these likely ghost positions and get the closest ghost
        distancesToGhosts = {}
        for ghostPosition in mostLikelyGhostPositions:
            distance = self.distancer.getDistance(
                pacmanPosition, ghostPosition)
            distancesToGhosts[ghostPosition] = distance
        closestGhostPosition = min(
            distancesToGhosts, key=distancesToGhosts.get)    # closest

        # for each legal moves, get the distance from the resulting successor position to the closest ghost
        actionDistances = {}
        for action in legal:
            newPos = Actions.getSuccessor(
                pacmanPosition, action)    # pacman's next position
            distanceToClosestGhost = self.distancer.getDistance(
                newPos, closestGhostPosition)    # ghost of new positino to ghost
            actionDistances[action] = distanceToClosestGhost
        # return the action of smallest distance
        return min(actionDistances, key=actionDistances.get)
